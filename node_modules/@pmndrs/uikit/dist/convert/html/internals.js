...
import { convertProperties, convertProperty, isInheritingProperty, toNumber, } from './properties.js';
...
export function parseHtml(text, colorMap) {
    ...
    const css = tailwindToCSS({
        ...
        .replaceAll(mediaQueryRegex, (_, breakpoint, content) => {
...
function convertParsedHtmlRecursive(element, classes, index, generate, colorMap, componentMap) {
    if (element instanceof HTMLElement && element.tagName?.toLowerCase() === 'svg') {
        ...
        const { inheritingProperties, properties, srOnly } = convertMergeSortProperties(false, [...conversionPropertyTypes.Icon, { svgWidth: ['number'], svgHeight: ['number'] }], classes, { svgWidth: toNumber(width) ?? 24, svgHeight: toNumber(height) ?? 24, text: element.toString() }, restAttributes, colorMap);
        if (srOnly) {
            return undefined;
        }
    ...
    switch (children) {
        ...
        case 'text':
            if (!(element instanceof TextNode)) {
                return generate(element, renderAs, custom, { ...inheritingProperties, ...properties }, index, element.childNodes
    ...
    let result = generate(element, renderAs, custom, properties ?? {}, index, element.childNodes
